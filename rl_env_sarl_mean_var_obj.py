...
class SARLDraftMeanVarEnv(DraftEnv):
    """
    ...
    def __init__(self, stochastic_temp, rl_mgr=1):
        super(SARLDraftMeanVarEnv, self).__init__(stochastic_temp=stochastic_temp)
        self.rl_mgr = rl_mgr  # which manager is the RL agent
        
    def reset(self, seed=None):
        '''needs to reset the draft and then run it until the RL agent's turn'''
        state, info = super(SARLDraftMeanVarEnv, self).reset(seed=seed)
