...
class DraftEnv(gym.Env):
    ...
    #     # first get what non-flex non kicker non def positions are needed
    #     needed_positions = [pos for pos, count in needed_pos_counts.items() if pos != "FLEX" and count > 0]
    #     # needed_pos_xF = [pos for pos, count in needed_pos_counts.items() if pos != "FLEX" and count > 0]
    #     # needed_pos_xK_xD_xF = [pos for pos, count in needed_pos_counts.items() if pos not in ["FLEX", "K", "DEF"] and count > 0]
        
    #     # if before round 8, ignore def and kicker
    #     if self.cur_round < 8:
    #         needed_pos_counts["K"] = 0
    #         needed_pos_counts["DEF"] = 0
